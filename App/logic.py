...
def new_logic():
    ...
    catalog['books_by_year_author'] = lp.new_map(100000,0.7)
...
def load_data(catalog):
    ...
    end_time = getTime()
    time_elapsed = deltaTime(end_time, start)
    memory_used = deltaMemory(start_memory, end_memory)
    return books, authors,tag_size,book_tag_size, time_elapsed, memory_used
...
def add_book_author_and_year(catalog, author_name, book):
    ...
    if author_value:
        ...
        if pub_year_value:
            ...
            pub_year_map = lp.new_map(100000,0.7)
            lp.put(pub_year_map,pub_year,book)
    else:
        author_map = lp.new_map(100000,0.7)
        books = al.new_list()
        al.add_last(books, book)
